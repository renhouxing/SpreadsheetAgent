...
async def async_cleanup_kernels(app, force=False):
    await asyncio.to_thread(cleanup_kernels, app, force)

class ExecuteHandler(tornado.web.RequestHandler):
    async def post(self):
        data = json.loads(self.request.body)
        convid = data.get("convid")
        mount_dir = data.get("mount_dir", "runs")
        code = data.get("code")
...
if __name__ == "__main__":
    ...
    periodic_cleanup = tornado.ioloop.PeriodicCallback(
        lambda: asyncio.create_task(async_cleanup_kernels(app)),
        int(os.environ.get("CLEANUP_TIMEOUT_MS", 300000))
    )
